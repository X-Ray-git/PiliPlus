...
- **快捷收藏操作**：
  ...
  - 支持多选收藏夹、批量添加/取消收藏
- **全局“稍后再看”状态同步**：
  - 引入了全局状态缓存服务 `LaterService`，重构了原本基于路由传参的脆弱逻辑。
  - 现已支持在所有界面（首页、动态、历史记录、详情页等）实时且无缝地同步视频是否在“稍后再看”列表中的状态。

### 修复与优化 (Fixes & Opts)

- **通知栏/后台播放控制**：
  - 修复了视频先开始播放、媒体信息稍后返回时，Android MediaSession 一直保留错误暂停状态，导致蓝牙耳机播放/暂停键始终无法暂停的问题。
  - 修复了通知栏恢复播放时，播放器回调缺失导致指令无响应的问题（回调不可用时退回播放器单例）。
- **构建脚本优化**：修复了 Github Action 自动打 Release 包时，默认排序混乱的问题（现已强制最新构建版本置顶展示）。
